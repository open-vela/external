/****************************************************************************
 * apps/external/glibc/port/string/test-string.h
 *
...
#define impls         STRINGIFY(main) "_impls"
#define __start_impls CONCATENATE(__start_, CONCATENATE(main, _impls))
#define __stop_impls  CONCATENATE(__stop_, CONCATENATE(main, _impls))
...
extern impl_t __stop_impls[];

#define IMPL(name, test)                                                    \
  const static impl_t tst_##name                                            \
      __attribute__((section(impls), used, aligned(sizeof(void *))))        \
...
#define CMDLINE_OPTIONS                                                     \
  ITERATIONS_OPTIONS{ "random", no_argument, NULL, OPT_RANDOM },            \
      { "seed", required_argument, NULL, OPT_SEED },

static void __attribute__((used)) cmdline_process_function(int c)
{
  switch (c)
    {
      ITERATIONS_PROCESS
        ...
        {                                                                   \
          ...
          for (impl = __start_impls; impl < __stop_impls; ++impl)           \
            if (strcmp(impl->name, TEST_NAME) == 0)                         \
              ...
              impl_count++;                                                 \
          a = impl_array                                                    \
              = xmalloc((impl_count + func_count) * sizeof(impl_t));        \
          for (impl = __start_impls; impl < __stop_impls; ++impl)           \
            if (impl != skip)                                               \
              *a++ = *impl;                                                 \
...
{
#ifdef TEST_NAME
  func_count = __libc_ifunc_impl_list(
      TEST_NAME, func_list, (sizeof func_list / sizeof func_list[0]));
#endif
